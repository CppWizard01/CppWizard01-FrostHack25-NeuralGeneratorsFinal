  ...
  if (message.trim()) {
    ...
    try {
      ...
      const response = await fetch(url, {
        method: selectedType === "music" ? "POST" : "GET",
        headers: { "Content-Type": "application/json" },
        body: selectedType === "music" ? JSON.stringify(requestBody) : null,
      });

      const data = await response.json();
      let newMessages = [{ text: message, isUser: true }];
      ...
      if (selectedType === "music" && data.audio_url) {
        ...
        newMessages.push({ text: `<audio controls><source src="${proxyUrl}" type="audio/mpeg">Your browser does not support the audio tag.</audio>`, isUser: false });
      } else if (selectedType === "image" && data.image_url) {
        newMessages.push({ imageUrl: data.image_url, isUser: false });
      } else if (selectedType === "text" && data.text) {
        newMessages.push({ text: data.text, isUser: false });
      } else {
        newMessages.push({ text: data.message || "No response received.", isUser: false });
      }
          ...
          <div className="sidebar-content">
            ...
            <ul>
              <li className="newbtn" onClick={handleNewChat}>
                <FontAwesomeIcon icon={faPlus} className="sidebar-icon " /> New Chat
              </li>
              <li className="history-item">
                <FontAwesomeIcon icon={faClockRotateLeft} /> History
              </li>
              {chatSessions.map((_, index) => (
                chatSessions[index] && chatSessions[index].length > 0 && <li key={index} className={index === activeSessionIndex ? "active-chat" : ""}>{sessionNames[index]} <FontAwesomeIcon icon={faTrash} className="delete-session" onClick={() => handleDeleteSession(index)} /></li>
              ))}
            </ul>
          </div>
        )}
      ...
      <div className={`main-content ${isSidebarOpen ? "with-sidebar" : "full-width"}`}>
        ...
        <div className="messages-container">
          {messages.map((message, index) => {
            return (
              <div key={index} className={`message-box ${message.isUser ? "user-message" : "generated-message"}`}>
                {message.imageUrl ? (
                  <div>
